...
public class Dungeon implements IDungeon {
  ...
  public void addThieves(int startPointLocationId) {
    ArrayList<Integer> notOccupiedNodes = new ArrayList<>();

    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
        if (dungeonGrid[i][j].getTreasure().size() > 0 || dungeonGrid[i][j].getArrowCount() > 0
                || dungeonGrid[i][j].getMonster() != null
                || dungeonGrid[i][j].getLocationId() == startPointLocationId) {
          continue;
        } else {
          notOccupiedNodes.add(dungeonGrid[i][j].getLocationId());
        }
      }
    }

    int numberOfThieves = RandomNumberGenerator.getRandomNumberInRange(1, 6);
    //    int numberOfThieves = RandomNumberGenerator.getFakeRandomNumber(2);

    int thiefCounter = 0;
    while (notOccupiedNodes.size() > 0 && thiefCounter < numberOfThieves) {
      int randomNode = RandomNumberGenerator.getRandomNumberInRange(0, notOccupiedNodes.size());
      //      int randomNode = RandomNumberGenerator.getFakeRandomNumber(0);
      int randomLocation = notOccupiedNodes.remove(randomNode);
      Location location = getLocationFromLocationId(randomLocation);
      if (dungeonGrid[location.getX()][location.getY()].getLocationType() == LocationType.CAVE) {
        dungeonGrid[location.getX()][location.getY()].addThief();
        thiefCounter++;
      }
    }
  }
  ...
  public void addPits(int startPointLocationId) {
    ...
    int numberOfPits = RandomNumberGenerator.getRandomNumberInRange(1, 4);
    //    int numberOfPits = RandomNumberGenerator.getFakeRandomNumber(2);

    int pitsCounter = 0;
    while (notOccupiedNodes.size() > 0 && pitsCounter < numberOfPits) {
      int randomNode = RandomNumberGenerator.getRandomNumberInRange(0, notOccupiedNodes.size());
      //      int randomNode = RandomNumberGenerator.getFakeRandomNumber(0);
      int randomLocation = notOccupiedNodes.remove(randomNode);
      Location location = getLocationFromLocationId(randomLocation);
      if (dungeonGrid[location.getX()][location.getY()].getLocationType() == LocationType.CAVE) {
        dungeonGrid[location.getX()][location.getY()].addPit();
        pitsCounter++;
      }
    }
  }
  ...
  private void createGridWithGivenConnectivityValue(int interconnectivity) {
    ...
    for (int i = 0; i < interconnectivity; i++) {
      ...
      //      ArrayList<Integer> singleEdge = this.remainingEdgesList.remove(mockRandom);
  ...
  private void addArrowsToDungeon(int numberOfNodesToBeFilledWithArrows) {
    ...
    for (int i = 0; i < this.rows; i++) {
      for (int j = 0; j < this.columns; j++) {
        nodeSet.add(this.dungeonGrid[i][j].getLocationId());
      }
    }
    ...
    while (countOfCavesAssignedWithTreasure != numberOfCavesToBeFilledWithTreasure) {
      ...
      if (this.dungeonGrid[locationOfRandomNode.getX()][locationOfRandomNode.getY()]
        ...
        for (int i = 0; i < 2; i++) {
          ...
          //          TreasureCatalog treasure = treasureCatalog.remove(mockRandom);
  ...
  public DungeonDump[][] getDungeonDump() {
    DungeonDump[][] dungeonDump = new DungeonDump[rows][columns];
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
        ArrayList<Pair> possibleMoves = this.dungeonGrid[i][j].getPossibleMoves();
        ArrayList<String> moves = new ArrayList<>();
        String directions = "";
        for (int k = 0; k < possibleMoves.size(); k++) {
          moves.add(possibleMoves.get(k).getDirectionFromPresentLocation().toString());
        }
        Collections.sort(moves);
        for (int m = 0; m < moves.size(); m++) {
          directions = directions + moves.get(m).charAt(0) + " ";
        }
        int locationId = this.dungeonGrid[i][j].getLocationId();
        Location location = getLocationFromLocationId(locationId);
        ArrayList<Treasure> treasureList = this.dungeonGrid[i][j].getTreasure();
        int arrowCount = this.dungeonGrid[i][j].getArrowCount();
        Otyugh monster = this.dungeonGrid[i][j].getMonster();
        boolean visited = this.dungeonGrid[i][j].getVisited();
        boolean hasThief = this.dungeonGrid[i][j].getHasThief();
        boolean hasPit = this.dungeonGrid[i][j].getHasPit();
        dungeonDump[i][j] = new DungeonDump(locationId, location, directions,
                treasureList, arrowCount, monster, visited, hasThief, hasPit);
      }
    }
    return dungeonDump;
  }
